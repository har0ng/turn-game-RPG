...
gevurah::gevurah() {}
gevurah::gevurah(const player& p) :player(p){ //magician status set
...
void gevurah::levelup() { // if level > 2 (+status) 
...
std::string gevurah::getClassName() { //자신의 직업에 대한 클래스 함수가 무엇인지 알기 위함, magician.cpp 니깐 클래스 함수는 magician
    return "gevurah";
}
