...
/* map -> button -> scene
1.map에서 한번에 층의 모든방의 정보를 만들어내기
2.그 정보를 button의 assortMapSelectbutton에 정보로 보내기
3.button에서 그 정보를 하나하나 읽어서 roomType에 따라 방의 이미지를 버튼화 시켜서 그 버튼을 scene으로 넘겨주기
4.층에 따라 세부 층에 있을 방의 개수별로 vector로 나눠 차곡차곡 저장
