...
std::vector<room> mapRoom::upperPartCreateMap() {
    ...
    bool hasRest = false; // 전체 맵에 최소 1개 이상 회복 보장하기 위함
    ...
    // (조건 3) 전체 맵에 회복이 하나도 없으면 랜덤 enemy 에서 rest 교체
    if (!hasRest) {
        for (auto& r : map) {
            if (r.name == "enemy") {
                r.name = "rest";
